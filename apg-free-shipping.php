...
$apg_free_shipping = array(	'plugin' => 'WooCommerce - APG Free Postcode/State/Country Shipping', 
								...
								'ajustes' => 'admin.php?page=woocommerce_settings&tab=shipping&section=apg_free_shipping', 
								'imagen' => 'http://www.artprojectgroup.es/wp-content/artprojectgroup/woocommerce-apg-free-postcodestatecountry-shipping-582x139.jpg', 
...
function apg_free_shipping_enlaces($enlaces, $archivo) {
	...
	if ($archivo == $plugin) 
	{
		...
		$enlaces[] = '<a href="mailto:info@artprojectgroup.es" title="' . __('Contact with us by ', 'apg_free_shipping') . 'e-mail"><span class="icon-open21"></span></a> <a href="skype:artprojectgroup" title="' . __('Contact with us by ', 'apg_free_shipping') . 'Skype"><span class="icon-social6"></span></a>';
...
function apg_free_shipping_inicio() {
	...
	class apg_free_shipping extends WC_Shipping_Method {				
		...
		function init_form_fields() {
			...
			if (get_option('woocommerce_enable_coupons') == 'yes' && $this->importe_minimo) $requerido = 'cualquiera';
			else if (get_option('woocommerce_enable_coupons') == 'yes') $requerido = 'cupon';
			else if ($this->importe_minimo) $requerido = 'importe_minimo';
			else $requerido = '';
		...
		function is_available($paquete) {
					...
					foreach ($woocommerce->cart->applied_coupons as $codigo)
					 {
						...
						if ($cupon->is_valid() && $cupon->enable_free_shipping()) $tiene_cupon = true;
					}
				}
			}
			...
			{
				if ($woocommerce->cart->prices_include_tax) $total = $woocommerce->cart->cart_contents_total + array_sum($woocommerce->cart->taxes);
				else $total = $woocommerce->cart->cart_contents_total;
				if ($total >= $this->importe_minimo) $tiene_importe_minimo = true;
			}
